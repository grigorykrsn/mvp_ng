...
export class AuthService implements OnDestroy {
  ...
  //   return from(
  //     this._authClient.signInWithCredentials({ username, password })
  //   ).pipe(map((t: AuthTransaction) => this.handleSignInResponse(t)));
  // }

  // public logout(redirect: string): Observable<void> {
  //   return from(this._authClient.signOut()).pipe(
  //     tap(
  //       (_) => (this._authSub$.next(false), this._router.navigate([redirect]))
  //     ),
  //     catchError((err) => {
  //       console.error(err);
